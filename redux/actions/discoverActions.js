...
export const update_movie_discovery_page = (page) => async dispatch => {
	let res = await fetch(`https://api.themoviedb.org/3/discover/movie?api_key=${API_KEY}&language=en-US&sort_by=popularity.desc&page=${page}&timezone=America%2FNew_York&include_null_first_air_dates=false`)
	let response = await res.json()
	.then((response) => dispatch({
